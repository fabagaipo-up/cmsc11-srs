 ...
 system("COLOR F0");
    ...
    while (menu >= 1 && menu <= 12) {
        ...
        if (menu >= 1 && menu <= 12) {
     		printf("\n\n\t\t==================================================MENU==================================================\n");
    		printf("\n\n\t\t==================WELCOME TO THE UNIVERSITY OF THE PHILIPPINES STUDENT REGISTRATION SYSTEM==============\n");
    		printf("\n\t\t\t\tPress 1. CREATE A STUDENT RECORD\n\t\t\t\tPress 2. UPDATE A STUDENT RECORD (STUDENT ID)\n\t\t\t\tPress 3. DISPLAY ALL STUDENT RECORDS\n\t\t\t\tPress 4. DELETE A STUDENT RECORD\n\t\t\t\tPress 5. ENROLL\n\t\t\t\tPress 6. SEARCH STUDENT RECORD\n\t\t\t\tPress 7. CREATE SUBJECT RECORD\n\t\t\t\tPress 8. UPDATE SUBJECT RECORD\n\t\t\t\tPress 9. DISPLAY SUBJECT RECORD\n\t\t\t\tPress 10. SEARCH SUBJECTS\n\t\t\t\tPress 11. DELETE SUBJECT\n\t\t\t\tPress 12. UPDATE SUBJECT\n\t\t\t\t");
    		printf("\n\t\t==================================================WARNING==================================================\n\t\t\t\t\t\t\tAny number outside 1 to 11. Exit\n\t\t===========================================================================================================\n");
    		printf("\t\t\t\t\t\t\tChoose one: ");
    		scanf("%d", &menu);
            fflush(stdin);
        } else {
...
struct student Update(){
    ...
    if (found == 1){
        while (choice>=1 && choice<=7){
            ...
            printf("\n\tHex 3: \t\t\t%.2f\n\n", cmsc11[i].hex3);
             printf("\n\tSubject 1: \t\t\t%.2f", cmsc11[i].sub1);
            printf("\n\tSubject 2: \t\t\t%.2f", cmsc11[i].sub2);
            printf("\n\tSubject 3: \t\t\t%.2f\n\n", cmsc11[i].sub3);
            printf("What do you want to update? Please choose from the choices (1-6) Enter [0] to go back to the Menu: \n\n1. Surname\n\n2. First Name\n\n3. CMSC11 Section\n\n4. Hex 1 Score\n\n5. Hex 2 Score\n\n6. Hex 3 Score\n[INPUT]: ");
            scanf(" %d", &choice);
            fflush(stdin);
            printf("\n================================================================================================================================================\nEdit:");
...
struct student Display(){
	...
	for (i=0; i<sNum; i++){
        ...
        tabs1(cmsc11[i].firstName);
        printf("\t%s", cmsc11[i].csSec);
        tabs2(cmsc11[i].csSec);
...
void FiletoStructSubject(){
    ...
    scanf(" %[^\n]s", &getFile);
    fflush(stdin);
	fptr = fopen(getFile,"r+");
	fclose(fptr);
    fptr = fopen(getFile,"a+");
	numOfdata=0;
	int i=0;

	fptr = fopen(getFile, "r+");
		while(!feof(fptr)) {
            fscanf(fptr,"%d,%[^,],%[^\n]",&data.subjectCode[numberOfSubs],data.subject[numberOfSubs],data.title[numberOfSubs]);
                    numberOfSubs++;
                    if(feof(fptr)){
                            break;
                    }
